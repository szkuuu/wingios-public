//
//  ChargeStandardModifyViewController.swift
//  WingUser
//
...
class ChargeStandardModifyViewController: UIViewController {
...
"""
  ...
  private lazy var nextButton: CTAButton = CTAButton(text: "충전규격 등록하기")

  let viewModel = ChargeStandardModifyViewModel(input: ChargeStandardModifyInput(), output: ChargeStandardModifyOutput())

  override func viewDidLoad() {
    super.viewDidLoad()
    view.backgroundColor = .systemBackground

    navigationItem.title = "충전규격 수정"
    ...
    viewModel.output.didSaveSuccessfully
      .withUnretained(self)
      .subscribe(onNext: { owner, _ in
        owner.alert(title: "충전정보 저장완료", message: "충전정보가 저장되었습니다.", style: .alert, actions: [
          UIAlertAction(title: "확인", style: .cancel) { _ in
    ...
    viewModel.output.didSaveUnsuccessfully
      .withUnretained(self)
      .subscribe(onNext: { owner, _ in
        owner.alert(title: "충전정보 저장실패", message: "충전정보가 저장되지 않았습니다. 다시 시도해주십시오.", style: .alert, actions: [
